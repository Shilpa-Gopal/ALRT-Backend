...
@app.route('/api/projects/<int:project_id>/check-training-readiness', methods=['GET'])
def check_training_readiness(project_id):
    """Check if project has enough balanced labeled data for training"""
    try:
        user_id = request.headers.get('X-User-Id')
        if not user_id:
            return jsonify({"error": "Unauthorized"}), 401
        ...
        # Get user-labeled citations after keyword filtering
        ...
        filtered_citations = apply_keyword_filtering(user_labeled_citations, project)
        ...
        can_train = len(filtered_citations) >= 10 and relevant_count >= 5 and irrelevant_count >= 5

        return jsonify({
            "can_train": can_train,
            "statistics": {
                "total_labeled": len(user_labeled_citations),
                "after_keyword_filtering": len(filtered_citations),
                "relevant_count": relevant_count,
                "irrelevant_count": irrelevant_count,
                "keyword_excluded": len(user_labeled_citations) - len(filtered_citations)
            },
            "requirements": {
                "min_total": 10,
                "min_relevant": 5,
            ...
            "needs_more_labeling": {
                "relevant": max(0, 5 - relevant_count),
                "irrelevant": max(0, 5 - irrelevant_count)
            }
        })

    except Exception as e:
        app.logger.error(f"Error checking training readiness: {str(e)}")
        return jsonify({"error": "Failed to check training readiness", "details": str(e)}), 500
...
@app.route('/api/projects/<int:project_id>/reset-iteration-with-keywords', methods=['POST'])
def reset_iteration_with_keywords(project_id):
    """Reset project iteration to 1 with updated keywords and handle labeled data"""
    try:
        user_id = request.headers.get('X-User-Id')
        if not user_id:
            return jsonify({"error": "Unauthorized"}), 401
        ...
        required_fields = ['keywords', 'keep_labeled_data', 'selected_citation_ids']
        if not all(field in data for field in required_fields):
            return jsonify({"error": f"Missing required fields: {required_fields}"}), 400

        new_keywords = data['keywords']
        keep_labeled_data = data['keep_labeled_data']
        selected_citation_ids = data['selected_citation_ids']

        # Validate keywords format
        if not isinstance(new_keywords, dict) or not all(k in new_keywords for k in ['include', 'exclude']):
            return jsonify({"error": "Invalid keywords format"}), 400

        # Validate exclude keywords format
        if not all(isinstance(item, dict) and 'word' in item and 'frequency' in item 
                  for item in new_keywords['exclude']):
            return jsonify({"error": "Invalid exclude keywords format"}), 400
        ...
        app.logger.info(f"Resetting iteration for project {project_id}. Keep labeled data: {keep_labeled_data}")

        if keep_labeled_data:
            # Reset all citations to unlabeled first
            Citation.query.filter_by(project_id=project_id).update({
                'is_relevant': None,
                'iteration': 0
            })

            # Then set selected citations as labeled for iteration 1
            if selected_citation_ids:
                for citation_data in selected_citation_ids:
                    citation_id = citation_data.get('id')
                    is_relevant = citation_data.get('is_relevant')
                    
                    if citation_id and is_relevant is not None:
                        citation = Citation.query.get(citation_id)
                        if citation and citation.project_id == project_id:
                            citation.is_relevant = is_relevant
                            citation.iteration = 1

            app.logger.info(f"Kept {len(selected_citation_ids)} labeled citations")
        else:
            # Clear all labels if not keeping labeled data
            Citation.query.filter_by(project_id=project_id).update({
                'is_relevant': None,
                'iteration': 0
            })
            app.logger.info("Cleared all labeled data")

        # Update project keywords and reset iteration
        project.keywords = new_keywords
        project.current_iteration = 1
        project.model_metrics = {}  # Clear previous model metrics

        db.session.commit()

        # Get updated citation counts after keyword filtering
        all_citations = Citation.query.filter_by(project_id=project_id, is_duplicate=False).all()
        filtered_citations = apply_keyword_filtering(all_citations, project)
        
        labeled_citations = [c for c in filtered_citations if c.is_relevant is not None]
        ...
        app.logger.info(f"Reset completed. Filtered citations: {len(filtered_citations)}, Labeled: {len(labeled_citations)} (R:{relevant_count}, IR:{irrelevant_count})")

        return jsonify({
            "message": "Project iteration reset successfully",
            "current_iteration": project.current_iteration,
            "keywords": project.keywords,
            "statistics": {
                "total_citations": len(filtered_citations),
                "labeled_citations": len(labeled_citations),
                "relevant_count": relevant_count,
                "irrelevant_count": irrelevant_count,
                "unlabeled_count": len(filtered_citations) - len(labeled_citations)
            },
            "data_action": "kept_labeled_data" if keep_labeled_data else "cleared_all_labels"
        })

    except Exception as e:
        app.logger.error(f"Error resetting iteration: {str(e)}")
        db.session.rollback()
        return jsonify({"error": "Failed to reset iteration", "details": str(e)}), 500
...
@app.route('/api/projects/<int:project_id>/labeled-citations-with-keywords', methods=['POST'])
def get_labeled_citations_with_keyword_preview(project_id):
    """Get labeled citations filtered by proposed keywords for reset preview"""
    try:
        user_id = request.headers.get('X-User-Id')
        if not user_id:
            return jsonify({"error": "Unauthorized"}), 401
        ...
        # Get all currently labeled citations
        ...
        # Create a temporary project object with new keywords for filtering
        temp_project = type('TempProject', (), {
            'keywords': new_keywords
        })()

        # Apply keyword filtering with new keywords
        filtered_labeled_citations = apply_keyword_filtering(labeled_citations, temp_project)

        # Count statistics
        relevant_count = sum(1 for c in filtered_labeled_citations if c.is_relevant == True)
        irrelevant_count = sum(1 for c in filtered_labeled_citations if c.is_relevant == False)
        
        return jsonify({
            "labeled_citations_after_keywords": [{
                "id": c.id,
                "title": c.title,
                "abstract": c.abstract,
                "is_relevant": c.is_relevant,
                "iteration": c.iteration
            } for c in filtered_labeled_citations],
            "statistics": {
                "total_originally_labeled": len(labeled_citations),
                "remaining_after_keywords": len(filtered_labeled_citations),
                "filtered_out_by_keywords": len(labeled_citations) - len(filtered_labeled_citations),
                "relevant_count": relevant_count,
                "irrelevant_count": irrelevant_count,
                "is_balanced": relevant_count >= 5 and irrelevant_count >= 5
            }
        })

    except Exception as e:
        app.logger.error(f"Error getting labeled citations with keyword preview: {str(e)}")
        return jsonify({"error": "Failed to get labeled citations preview", "details": str(e)}), 500
